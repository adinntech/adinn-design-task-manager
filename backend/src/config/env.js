...
function validateEnv() {
  ...
  if (['supabase', 'supabase_relational'].includes(env.dataDriver)) {
    if (!process.env.SUPABASE_URL) throw new Error('SUPABASE_URL is required when DATA_DRIVER=supabase or supabase_relational.');
    if (!process.env.SUPABASE_SERVICE_ROLE_KEY) throw new Error('SUPABASE_SERVICE_ROLE_KEY is required when DATA_DRIVER=supabase or supabase_relational.');
  }
