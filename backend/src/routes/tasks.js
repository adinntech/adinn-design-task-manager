...
const { canSeeTask, enrichTask, applyComputedStatus, statusLabel, safePerson } = require('../utils/tasks');
...
function makeTaskListContext(db) {
  const usersById = new Map(db.users.map((user) => [user.id, user]));
  const fileCountByTask = new Map();
  for (const file of db.task_files || []) {
    fileCountByTask.set(file.task_id, (fileCountByTask.get(file.task_id) || 0) + 1);
  }
  return { usersById, fileCountByTask };
}

function taskVerticalFast(task, db, usersById) {
  if (task.vertical) return task.vertical;
  const designer = usersById.get(task.assigned_to);
  return normalizeVerticals(designer?.verticals)[0] || '';
}

function enrichTaskListItem(task, context) {
  return {
    ...task,
    status_label: statusLabel(task.status),
    computed_status_label: statusLabel(task.computed_status || task.status),
    assigner: safePerson(context.usersById.get(task.assigned_by)),
    designer: safePerson(context.usersById.get(task.assigned_to)),
    rater: safePerson(context.usersById.get(task.rated_by)),
    file_count: context.fileCountByTask.get(task.id) || 0
  };
}
...
router.get('/', asyncRoute(async (req, res) => {
  ...
  const paged = tasks.slice(start, start + limit).map((task) => enrichTaskListItem(task, context));

  res.json({ tasks: paged, total, page, limit, total_pages: Math.max(1, Math.ceil(total / limit)) });
}));
...
router.post('/', permit('admin', 'superadmin', 'bd', 'manager'), upload.array('files', 20), asyncRoute(async (req, res) => {
  const {
    assignment_date,
    assignment_time,
    task_title,
  ...
  if (!task_title || !client_name || !category || !assigned_to || !vertical || !priority || !deadline_date || !deadline_time) {
    return res.status(400).json({ message: 'Task, client name, task category, designer, vertical, priority, and deadline date/time are required.' });
  }
  ...
  const task = await updateDb(async (db) => {
    ...
    const taskRecord = {
      ...
      assignment_time: assignment_time || now().slice(11, 16),
      task_title,
...
router.patch('/:id/rating', permit('admin', 'superadmin', 'bd'), asyncRoute(async (req, res) => {
...
router.patch('/:id/reassign', permit('admin', 'superadmin', 'bd'), asyncRoute(async (req, res) => {
