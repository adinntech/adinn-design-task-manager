{{-- BD Completed Task Ratings rows + pagination — swapped in/out by its own Designer filter/pager. No <style>/<script>, safe to AJAX-swap. --}}
@php
            ...
            <div class="dhr-row">
                <div class="dhr-col dhr-col-task">
                    <div class="dhr-task-name">
                        @if($task)
                            <a class="dh-task-link" href="{{ route('bd.tasks.show', $task) }}">{{ $task->task_id }}</a> — {{ $task->display_task_name ?? $task->task_name }}
                        @else
                            Task unavailable
                        @endif
                    </div>
