...
@section('workspace-title','User Management')
...
<div class="panel">
    <div class="panel-body">
        <form method="GET" class="filter-bar compact" style="margin-bottom:14px">
            <input class="premium-input" name="search" value="{{ request('search') }}" placeholder="Search name or email">

            <select class="premium-select" name="role">
                <option value="">All Roles</option>
                @foreach(['admin'=>'Admin','bd'=>'BD','designer'=>'Designer','designer_head'=>'Designer Head'] as $k=>$v)
                ...
                <tbody>
                    @forelse($users as $user)
                        <tr>
                            ...
                            <td>{{ $user->email }}</td>
                            <td><span class="badge badge-dark">{{ ucwords(str_replace('_',' ',$user->role)) }}</span></td>
                            <td>
                                ...
                                <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
                                    ...
                                    @if(!$user->is(auth()->user()))
                                        <form method="POST" action="{{ route('admin.users.toggle',$user) }}">
                                            @csrf
                                            @method('PATCH')
                                            <button class="btn {{ $user->is_active ? 'btn-danger' : 'btn-secondary' }}">
                                    ...
                                    @else
                                        <span style="font-size:9px;color:#98a2b3;font-weight:700">Current Admin</span>
                                    @endif
                                </div>
                            </td>
                        </tr>
