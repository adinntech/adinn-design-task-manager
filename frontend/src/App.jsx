...
function TasksPage({ user, notify }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [taskResponse, userResponse, metaResponse] = await Promise.all([
        api.getTasks({ ...filters, page: taskPage, limit: 50 }),
        api.getUsers({ role: 'designer', status: 'active', vertical: filters.vertical }),
        api.getTaskMeta()
      ]);
      setTasks(taskResponse.tasks);
      setTaskTotal(taskResponse.total || taskResponse.tasks.length);
      setTaskPages(taskResponse.total_pages || 1);
  ...
  }, [filters, taskPage, notify]);

  useEffect(() => { load(); }, [load]);
  useEffect(() => { setTaskPage(1); }, [filters]);
              ...
              <tbody>
                {tasks.map((task) => (
                  <tr key={task.id} onClick={() => openTask(task.id)}>
                    <td>{task.assignment_date ? formatDate(task.assignment_date, task.assignment_time) : String(task.created_at || '').slice(0, 10)}</td>
                    <td>
                    ...
                    <td>{task.file_count ?? task.files?.length ?? 0}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function CreateTaskPage({ user, notify, setPage }) {
  ...
  const [loading, setLoading] = useState(false);
  const [assignmentDateTime] = useState(() => new Date());
  ...
  async function submit(event) {
    ...
    setLoading(true);
    try {
      const [deadlineDate = '', deadlineTime = ''] = String(form.deadline_datetime || '').split('T');
      const fd = new FormData();
      fd.append('assignment_date', toDateInputValue(assignmentDateTime));
      fd.append('assignment_time', toTimeInputValue(assignmentDateTime));
      fd.append('task_title', form.task_title.trim());
      fd.append('client_name', form.client_name.trim());
      fd.append('zoho_project_no', form.zoho_project_no.trim());
      fd.append('category', form.category);
      fd.append('assigned_to', form.assigned_to);
      fd.append('vertical', form.vertical);
      fd.append('priority', form.priority);
      fd.append('deadline_date', deadlineDate);
      fd.append('deadline_time', deadlineTime);
  ...
  const assignedByName = user?.name || user?.email || 'Current user';

  return (
    <>
      <PageHeader
        title="Create Task"
        subtitle="Create a task using only the required assignment details. Assignment date/time and assigned by are captured automatically."
      ...
      <form className="panel formGrid" onSubmit={submit}>
        <Field label="Date & Time of Assignment"><input value={formatDateTime(assignmentDateTime)} readOnly /></Field>
        <Field label="Assigned By"><input value={assignedByName} readOnly /></Field>
        <Field label="TASK" required><input required value={form.task_title} onChange={(e) => setField('task_title', e.target.value)} placeholder="Example: Premium real estate hoarding design" /></Field>
        <Field label="Client Name" required><input required value={form.client_name} onChange={(e) => setField('client_name', e.target.value)} placeholder="Client or brand name" /></Field>
        ...
        <Field label="Designers" required>
          <select required value={form.assigned_to} onChange={(e) => { const designer = designers.find((item) => item.id === e.target.value); setForm((current) => ({ ...current, assigned_to: e.target.value, vertical: designer?.verticals?.[0] || current.vertical })); }}>
            <option value="">Select designer</option>
            {designers.map((u) => <option key={u.id} value={u.id}>{u.name} - {u.active_count} current - {verticalText(u.verticals)}</option>)}
          </select>
        </Field>
        ...
        <Field label="Priority" required><select required value={form.priority} onChange={(e) => setField('priority', e.target.value)}>{(meta?.priorities || ['Low', 'Medium', 'High', 'Urgent']).map((p) => <option key={p} value={p}>{p}</option>)}</select></Field>
        <Field label="Deadline Date & Time" required><input required type="datetime-local" value={form.deadline_datetime} onChange={(e) => setField('deadline_datetime', e.target.value)} min={toDateTimeLocalValue(new Date())} /></Field>
        <Field label="Description" wide><textarea value={form.description} onChange={(e) => setField('description', e.target.value)} rows="5" placeholder="Optional: creative direction, sizes, references, content, mandatory logo usage, etc." /></Field>
    ...
    <div className="drawerBackdrop" onClick={close}>
      <aside className="drawer" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="detailGrid">
          <Info label="Date of Assignment" value={task.assignment_date ? formatDate(task.assignment_date, task.assignment_time) : formatDateTime(task.created_at)} />
...
function UsersPage({ user, notify }) {
  ...
  useEffect(() => { load(); }, [load]);
  useEffect(() => { setUserPage(1); }, [filters]);
...
function ReportsPage({ notify }) {
  ...
  const [designers, setDesigners] = useState([]);
  const [loading, setLoading] = useState(true);
  const [reportPages, setReportPages] = useState({ period: 1, project: 1, designer: 1 });
  ...
  useEffect(() => { load(); }, [load]);
  useEffect(() => { setReportPages({ period: 1, project: 1, designer: 1 }); }, [filters]);
  ...
  const periodPages = Math.max(1, Math.ceil(byPeriod.length / reportPageSize));
  const projectPages = Math.max(1, Math.ceil(byProject.length / reportPageSize));
  const designerPages = Math.max(1, Math.ceil(byDesigner.length / reportPageSize));
  const pagedPeriod = byPeriod.slice((reportPages.period - 1) * reportPageSize, reportPages.period * reportPageSize);
  const pagedProject = byProject.slice((reportPages.project - 1) * reportPageSize, reportPages.project * reportPageSize);
  const pagedDesigner = byDesigner.slice((reportPages.designer - 1) * reportPageSize, reportPages.designer * reportPageSize);
  ...
  return (
    <>
      ...
      <section className="panel tablePanel">
        ...
        <Pagination page={reportPages.period} totalPages={periodPages} total={byPeriod.length} label="period rows" onPage={(value) => setReportPages((current) => ({ ...current, period: value }))} />
      </section>
      ...
      <section className="panel tablePanel">
        ...
        <Pagination page={reportPages.project} totalPages={projectPages} total={byProject.length} label="project rows" onPage={(value) => setReportPages((current) => ({ ...current, project: value }))} />
      </section>
      ...
      <section className="panel tablePanel">
        ...
        <Pagination page={reportPages.designer} totalPages={designerPages} total={byDesigner.length} label="designer rows" onPage={(value) => setReportPages((current) => ({ ...current, designer: value }))} />
      </section>
      <section className="panel">
...
function Pagination({ page, totalPages, total, label, onPage }) {
  ...
  return (
    <div className="paginationBar">
      <button type="button" className="secondaryBtn" disabled={safePage <= 1} onClick={() => onPage(1)}>First</button>
      <button type="button" className="secondaryBtn" disabled={safePage <= 1} onClick={() => onPage(safePage - 1)}>Prev</button>
      <span>Page {safePage} of {safeTotalPages}{Number.isFinite(Number(total)) ? ` • ${total} ${label || 'records'}` : ''}</span>
      <button type="button" className="secondaryBtn" disabled={safePage >= safeTotalPages} onClick={() => onPage(safePage + 1)}>Next</button>
      <button type="button" className="secondaryBtn" disabled={safePage >= safeTotalPages} onClick={() => onPage(safeTotalPages)}>Last</button>
    </div>
  );
}
