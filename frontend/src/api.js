...
const API_CACHE_TTL_MS = Number(import.meta.env.VITE_API_CACHE_TTL_MS || 10000);
...
async function request(path, options = {}) {
  ...
  const cacheable = method === 'GET' && !options.raw;
  const cacheKey = `${method}:${path}`;

  if (cacheable) {
    const cached = responseCache.get(cacheKey);
    if (cached && (Date.now() - cached.time) < API_CACHE_TTL_MS) return cloneData(cached.data);
    if (inFlightRequests.has(cacheKey)) return cloneData(await inFlightRequests.get(cacheKey));
  }
  ...
  const run = (async () => {
    ...
    if (cacheable) responseCache.set(cacheKey, { data: cloneData(data), time: Date.now() });
    if (!cacheable && method !== 'GET') clearApiCache();
    return data;
  })();
